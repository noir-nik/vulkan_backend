...
template <typename T>
concept MatrixType =
	TypeIn<float16_t, float, int8_t, int16_t, int32_t, int64_t, uint8_t, uint16_t, uint32_t, uint64_t>::template value<T>;
...
struct Matrix : public MatrixBase {
	...
	inline size_t RavelIndex(size_t row, size_t col) const { return row * cols + col; }
...
void MatMulAdd(Matrix<AType> const& matA, Matrix<AType> const& matB, Matrix<ResultType> const& matC, Matrix<ResultType>& matD,
			   ResultType const alpha, ResultType const beta) {
